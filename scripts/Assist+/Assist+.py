...
def get_stats_for_season(season, games_filter=20, assist_filter=4, override_file=False):
    ...
    tracking_df = tracking_ep.get_data({'Season': season, 'PerMode': 'Totals', 'PtMeasureType': 'Passing'},
...
def get_stats_for_year_range(year_range):
    assist_df = pd.DataFrame()
    for year in year_range:
        season = get_year_string(year)
        assist_df = assist_df.append(get_stats_for_season(season))
        assist_df = assist_df.sort_values(by='PTS_PER_PAST', ascending=False)
    return assist_df
